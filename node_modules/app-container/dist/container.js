...
const debug = (0, _debug2.default)('app-container');
const PLUGIN = /^(all|any|container)!.*/;
...
class Container {
  ...
  glob(pattern, options = {}) {
    ...
    const filenames = _glob2.default.sync(pattern, options);
    ...
    filenames.forEach(filename => {
      ...
      const result = (0, _lodash.attempt)(() => {
  ...
  load(...components) {
    return _bluebird2.default.try(() => {
      ...
      }if (components.length === 1 && typeof components[0] === 'object') {
        return this._loadRecursive(components[0]);
      }if (Array.isArray(components)) {
        return _bluebird2.default.mapSeries(components, name => this._load(name));
      }
   ...
   */
  _loadAny(pattern) {
    return this._loadAll(pattern).then(deps => Object.keys(deps).reduce((memo, name) => [...memo, deps[name]], []));
  }
  ...
  _registerNamespaced(mod, name, options) {
    (0, _lodash.defaults)(options, this.options.defaults || {});
    let main = mod;
    if (mod.__esModule && mod.default) {
      main = mod.default;
    }
    const component = new _component2.default({ mod: main, name, options });
  ...
  _validateDependencyGraph() {
    ...
    const graph = new _dependencyGraph.DepGraph();
    ...
    overallOrder.forEach(name => {
      if (!PLUGIN.test(name) && !(this.registry[name] instanceof _component2.default)) {
...
function _loadRecursive(container, map, accum = {}) {
  return _bluebird2.default.reduce(Object.keys(map), (memo, key) => {
    const path = map[key];
    if (typeof path === 'string') {
      return container._load(path).then(mod => {
        (0, _lodash.set)(memo, key, mod);
        return memo;
      });
    }
    return _loadRecursive(container, path).then(mods => {
      (0, _lodash.set)(memo, key, mods);
      return memo;
    });
  }, accum);
}
