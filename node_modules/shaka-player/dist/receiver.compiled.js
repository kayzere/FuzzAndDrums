...
*/
goog.asserts={};goog.asserts.ENABLE_ASSERTS=goog.DEBUG;goog.asserts.assert=function(){};goog.asserts.patchAssert_=function(){var a=console.assert;a?a.bind||(console.assert=function(){a.apply(console,arguments)}):console.assert=function(){}};goog.asserts.ENABLE_ASSERTS&&(goog.asserts.patchAssert_(),goog.asserts.assert=console.assert.bind(console));function ShakaReceiver(){this.idleTimerId_=this.idle_=this.receiver_=this.player_=this.video_=null;this.idleTimeout_=300}
ShakaReceiver.prototype.init=function(){shaka.polyfill.installAll();var a=document.getElementById("video");goog.asserts.assert(a,"Video element should be available!");this.video_=a;a=this.video_.ui;goog.asserts.assert(a,"UI should be available!");a.configure({fadeDelay:3,controlPanelElements:["play_pause","time_and_duration","spacer"]});this.player_=a.getControls().getLocalPlayer();goog.asserts.assert(this.player_,"Player should be available!");this.idle_=document.getElementById("idle");this.receiver_=
new shaka.cast.CastReceiver(this.video_,this.player_,this.appDataCallback_.bind(this));this.receiver_.addEventListener("caststatuschanged",this.checkIdle_.bind(this));this.startIdleTimer_()};ShakaReceiver.prototype.appDataCallback_=function(a){a&&(a=ShakaDemoAssetInfo.fromJSON(a.asset),a.applyFilters(this.player_.getNetworkingEngine()),a=a.getConfiguration(),this.player_.configure(a))};
...
ShakaReceiver.prototype.startIdleTimer_=function(){this.cancelIdleTimer_();this.idleTimerId_=window.setTimeout(window.close.bind(window),1E3*this.idleTimeout_)};ShakaReceiver.prototype.cancelIdleTimer_=function(){null!=this.idleTimerId_&&(window.clearTimeout(this.idleTimerId_),this.idleTimerId_=null)};function receiverAppInit(){window.receiver=new ShakaReceiver;window.receiver.init()}document.addEventListener("shaka-ui-loaded",receiverAppInit);var ShakaDemoAssetInfo=function(a,b,c,d){this.name=a;this.shortName="";this.iconUri=b;this.manifestUri=c;this.source=d;this.disabled=this.focus=!1;this.extraText=[];this.certificateUri=null;this.description="";this.isFeatured=!1;this.drm=[shakaAssets.KeySystem.CLEAR];this.features=[];this.licenseServers=new Map;this.licenseRequestHeaders=new Map;this.drmCallback=this.responseFilter=this.requestFilter=null;this.clearKeys=new Map;this.extraConfig=null;this.storedProgress=1};
